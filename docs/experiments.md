...
|---|---|---|---|---|---|---|---|---|
| E010-anchors12 | 2026-09-04 | blended_v1 soft labels + tier weights / 4,407 / 4 slots | E009 config with n_anchors 8 -> 12 (near-full slice coverage), nothing else moved | fixed 90/10 holdout (seed 0, paired with E009) | 0.799 (null vs E009's 0.803; gate failed) | — (not submitted) | train ~4h T4 | this file (log below), kernels train v14 |
...
- **Outcome (2026-09-04, kernel v14, ~4h): NULL — 0.799 (epoch 13) vs E009's 0.803; gate failed, not submitted.** -0.004 is within noise on the 441-study holdout, and the per-label churn nets to zero: menisci/MCL up (Medial Meniscus 0.770 -> 0.789, MCL 0.758 -> 0.770, Baker's 0.845 -> 0.868), effusion-family down (Effusion 0.827 -> 0.806, Synovitis 0.819 -> 0.802, Contusion 0.823 -> 0.788). Best explanation for the null: anchor JITTER already gives full slice coverage *across epochs* during training — 8 vs 12 anchors only changes per-forward coverage, which the attention head apparently doesn't need. The E005 full-slice evidence was a frozen-bank regime result that doesn't transfer to the fine-tuned bag model. Coverage is now ruled out as a bottleneck; remaining levers are resolution/ROI (thin structures), the label table, and endgame ensembling.
